...
router.get('/', optionalAuth, async (req,res) => {
    try {
        ...
        if(books.length == 0){
           return res.status(200).json({ message: 'no book added yet' })
        }
...
router.get('/aggregation', requireAuth, async (req, res) => {
  try {
...
router.post('/', requireAuth, async(req,res) => {
    try {
        ...
        const addedBook = await Book.create({
            ...
            year,
            isbn,
            userId: req.user.id   // ✅ tie book to logged-in user
        })
        addedBook.save()
        if(!addedBook){
...
router.put('/:id', requireAuth, async(req,res,next) => {
    ...
    req.resource = { ownerId: book.userId.toString() }
    next()
}, requireOwnership, async(req,res) => {
    try {
       const updatedInfo = await Book.findByIdAndUpdate(req.params.id, req.body, {new:true})
       if(!updatedInfo){
        return res.status(500).json({ message: 'something went wrong while updating book' })
       }
       return res.status(200).json({ data: updatedInfo, message: 'Book updated successfully' })
    } catch (error) {
